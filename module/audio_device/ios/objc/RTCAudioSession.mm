...
NSString * const kRTCAudioSessionErrorDomain = @"ss181.app.RTCAudioSession";
...
- (BOOL)setActive:(BOOL)active
  ...
  if (!active && activationCount == 0) {
      NSLog(@"Attempting to deactivate without prior activation.");
  }
  BOOL success = YES;
  ...
  if (success) {
    ...
    if (active) {
      ...
      NSLog(@"Failed to setActive:%d. Error: %@", active, error);
  }
  // Decrement activation count on deactivation whether or not it succeeded.
  if (!active) {
    [self decrementActivationCount];
  }
    NSLog(@"Number of current activations: %ld", (long)self.activationCount);
  return success;
}
...
- (NSInteger)incrementActivationCount {
  NSLog(@"Incrementing activation count.");
  @synchronized(self) {
    return ++_activationCount;
  }
}
...
- (NSInteger)decrementActivationCount {
  NSLog(@"Decrementing activation count.");
  @synchronized(self) {
    return --_activationCount;
  }
}
...
- (void)updateAudioSessionAfterEvent {
  ...
  if ([self.session setActive:shouldActivate
    ...
    NSLog(@"Failed to set session active to %d. Error:%@",
